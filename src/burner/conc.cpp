...
static std::vector<char> CurrentMameCheatContent; // Global
static std::vector<char> CurrentIniCheatContent; // Global
static int usedCheatType = 0; //Global so we'll know if cheatload is already done or which cheat type it uses?
...
static INT32 LoadIniContentFromZip(const char* DrvName, const char* zipFileName, std::vector<char>& iniContent) {
	...
	if (ZipOpenSpecificType((char*)zipCheatPath) != 0) {
		ZipCloseSpecificType();
		return 1;
	}
	...
	if (ZipGetListSpecificType(&pList, &pnListCount) != 0) {
		ZipCloseSpecificType();
		return 1;
	}
	...
	for (int i = 0; i < pnListCount; i++) {
		if (strcmp(pList[i].szName, iniFileName) == 0) {
			...
			INT32 pnWrote = 0;
			if (ZipLoadFileSpecificType((UINT8*)dest, pList[i].nLen, &pnWrote, i) == 0) {
...
static bool bIsChosenChanged = false;
static std::string PreviousChosenFileValue = "";
INT32 ConfigCheatLoad() {
	TCHAR szFilename[MAX_PATH] = _T("");
	TCHAR szFilenameExist[MAX_PATH] = _T("");
	std::string ChosenFileValue = "";
	bool bisDefaultCheatfileValue = true;
	INT32 ret = 1;

	if (CheckCheatFilesExist()) {
		for (int i = 0; i < cheatfilename_size; i++) {
			sprintf(szFilenameExist, "%s%s.SetToLoad", szAppCheatsPath, cheatfilename[i]);
			FILE* SetToLoadfileExist = _tfopen(szFilenameExist, _T("rb"));
			if (SetToLoadfileExist != NULL) {
				fclose(SetToLoadfileExist);
				ChosenFileValue = cheatfilename[i];
				bisDefaultCheatfileValue = false;
			}
		}

		bIsChosenChanged = (ChosenFileValue != PreviousChosenFileValue);

		if (bIsChosenChanged)
		{
			usedCheatType = 0;
			PreviousChosenFileValue = ChosenFileValue;
		}
	}
	...
	switch (usedCheatType) {
		case 0:
			if (bisDefaultCheatfileValue) {
				if (ConfigParseMAMEFile()) {
					ret = ExtractIniFromZip(BurnDrvGetText(DRV_NAME), "cheat.zip", CurrentIniCheatContent);
					if (ret == 0) {
						sprintf(szFilename, "%s%s.ini(cheat.zip)", szAppCheatsPath, BurnDrvGetText(DRV_NAME));
						ret = ConfigParseFile(szFilename, &CurrentIniCheatContent);
					}
					#ifdef INCLUDE_7Z_SUPPORT
					if (ret > 0) {
						ret = ExtractIniFromZip(BurnDrvGetText(DRV_NAME), "cheat.7z", CurrentIniCheatContent);
						if (ret == 0) {
							sprintf(szFilename, "%s%s.ini(cheat.7z)", szAppCheatsPath, BurnDrvGetText(DRV_NAME));
							ret = ConfigParseFile(szFilename, &CurrentIniCheatContent);
						}
					}
					#endif
			...
			if (ChosenFileValue == "cheat.dat") {
				ret = ConfigParseMAMEFile();
			}

			if (ChosenFileValue == "cheat.zip") {
				ret = ExtractIniFromZip(BurnDrvGetText(DRV_NAME), "cheat.zip", CurrentIniCheatContent);
				if (ret == 0) {
					sprintf(szFilename, "%s%s.ini(cheat.zip)", szAppCheatsPath, BurnDrvGetText(DRV_NAME));
					ret = ConfigParseFile(szFilename, &CurrentIniCheatContent);
